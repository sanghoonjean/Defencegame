...
스폰 지점에서 기지까지 타워를 장애물로 피해가는 A* 최단경로로 이동하여
기지에 도달하면 플레이어 HP를 감소시킨다 (#326).
...
### 4. 이동
- PathfindingSystem이 실시간 계산한 A* 최단경로를 따라 이동 (타워를 장애물로 회피) (#326)
- 타워가 배치·이동·삭제될 때마다 살아있는 적의 경로도 현재 위치 기준으로 재계산
- 이동속도는 등급별 스탯 적용
