# Issue #222 — [FEAT] 단축키로 타워 삭제 (Delete/Backspace)

> #218 타워 삭제 기능의 후속. 마우스 클릭 대신 키보드 단축키로 삭제 확인 팝업을 띄우고, 팝업 내에서 Enter/Esc 로 빠르게 확정/취소한다. 다수 타워 정리 시 UX 개선.
...
[TowerDeleteConfirmPopup.Update]  ← 본 작업에서 추가
   ...
   │    ├─ Delete OR Backspace 눌림 + 입력 가드 통과
...
| Delete + Backspace 동시 지원 | **둘 다 받음** (이슈 요구사항). Mac 키보드는 Delete 키가 없고 Backspace가 그 역할 |
| Enter 키 | `KeyCode.Return` **+** `KeyCode.KeypadEnter` 둘 다 받음 | 넘버패드 Enter도 자연스럽게 동작해야 함 |
| 즉시 삭제 X | **반드시 팝업 경유** | 이슈 위험 요소 — 오삭제 방지. Delete 키는 팝업을 띄울 뿐 삭제 X |
| 텍스트 입력 가드 | **`EventSystem.current.currentSelectedGameObject` 의 `InputField`/`TMP_InputField` 보유 시 무시** | 이슈 요구사항. 현재 코드베이스에 InputField는 없지만 향후 채팅/이름입력 도입 대비 |
| 팝업 오픈 중 트리거 키 무시 | **`panel.activeSelf == true` 면 Delete/Backspace 무시** | 팝업이 이미 열려 있으면 Delete 키로 새 팝업 트리거하지 않음 (현 팝업 대상이 무엇이었는지 혼동 방지) |
| 키 충돌 검사 | **없음** | `KeyCode.Delete/Backspace/Return/KeypadEnter/Escape` grep → 0 hit. 기존 `TestRunner`는 `Space/A/C/R` 만 사용 |
...
1. **트리거 동작**
   - [ ] 타워 미선택 + `Delete` → 팝업 안 뜸 (`SelectedTower == null` 가드)
   - [ ] 타워 선택 + `Delete` → 팝업 표시
   - [ ] 타워 선택 + `Backspace` → 팝업 표시
   - [ ] 팝업이 이미 열린 상태 + `Delete` → 무시 (중복 트리거 없음)
2. **팝업 내 키 처리**
...
3. **입력 가드** (수동, 임시 InputField 추가하여 검증)
   - [ ] Hierarchy에 임시 `InputField` 추가 → 포커스 → `Delete`/`Backspace` 입력 → 팝업 안 뜸, InputField 글자만 삭제
   - [ ] `TMP_InputField` 동일 확인 (TextMeshPro 패키지 사용 시)
   - [ ] InputField 포커스 해제 (다른 곳 클릭) → `Delete` 정상 동작
4. **사이드 이펙트**
   - [ ] 팝업 오픈 중 다른 타워 클릭(`InventorySystem.SelectTower`) → 캡처된 원본 타워 삭제 (#218 캡처 패턴 유지 — 본 작업 무관)
   - [ ] 웨이브 진행 중 `Delete` → 팝업 정상, 확정 시 적 추적 영향 없음
...
- **포커스 가드의 보수성** — `InputField`가 포커스됐어도 `currentSelectedGameObject`에 그대로 남아있지 않은 케이스(Tab 키로 포커스 이동 후 등)는 가드 누락 가능. 현재 코드베이스에 InputField가 없으므로 즉시 문제는 없으나, 향후 도입 시 추가 가드(`UnityEngine.UI.InputField.GetComponent<InputField>().isFocused`)를 검토.
