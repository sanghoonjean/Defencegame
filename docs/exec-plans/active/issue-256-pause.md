...
> **공통 사전조건**: 1~7번 토글/시뮬레이션 시나리오는 진입 게이트 (`IsWaveActive == true`) 통과를 위해 **먼저 `Space` 로 웨이브를 시작한 뒤** 진행. Play 시작 직후 비활성 시점의 토글 무반응은 별도 8번(진입 게이트) 에서 검증.

1. **토글 동작** (웨이브 진행 중 가정)
   - [ ] 웨이브 시작 → HUD 일시정지 라벨 `⏸` (=현재 재생 중이라 클릭 시 정지) 표시
   - [ ] 버튼 클릭 → 라벨 `▶`, `Time.timeScale == 0`
   - [ ] 다시 클릭 → 라벨 `⏸`, `Time.timeScale == 1` (또는 직전 배속)
2. **단축키** (웨이브 진행 중)
   - [ ] `P` → 동일 토글
3. **시뮬레이션 정지**
